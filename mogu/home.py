...
def main():
    ...
    cursor = db.cursor()
    for k in range(1000):
        url = 'https://list.mogujie.com/search?callback=jQuery211034720169596246464_1540374493143&_version=8193&ratio=3%3A4&cKey=15&page='+ str(k+1)+str('&sort=pop&ad=0&fcid=51642&action=home')
        ...
        for i in range(len(docs)):
            ...
            sql = """insert into home (title,orgPrice,sale,cfav,price,tradeItemId) values('%s','%s','%s','%s','%s','%s')""" % (title, orgPrice, sale, cfav, price, tradeItemId)
            print(sql)
            cursor.execute(sql)
            db.commit()
