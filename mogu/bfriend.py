...
def main():
    ...
    cursor = db.cursor()
    for k in range(2000):
        url = 'https://list.mogujie.com/search?callback=jQuery21108092191159507773_1540372891702&_version=8193&ratio=3%3A4&cKey=15&page='+ str(k+1)+str('&sort=pop&ad=0&fcid=51716&action=boyfriend')
        ...
        for i in range(len(docs)):
            ...
            sql = """insert into bfriend (title,orgPrice,sale,cfav,price,tradeItemId) values('%s','%s','%s','%s','%s','%s')""" % (title, orgPrice, sale, cfav, price, tradeItemId)
            print(sql)
            cursor.execute(sql)
            db.commit()
