...
def main():
    ...
    cursor = db.cursor()
    for k in range(1000):
        url = 'https://list.mogujie.com/search?callback=jQuery21105437357018603932_1540373009434&_version=8193&ratio=3%3A4&cKey=15&page='+ str(k+1)+str('&sort=pop&ad=0&fcid=10056587&action=sports')
        ...
        for i in range(len(docs)):
            ...
            sql = """insert into sport (title,orgPrice,sale,cfav,price,tradeItemId) values('%s','%s','%s','%s','%s','%s')""" % (title, orgPrice, sale, cfav, price, tradeItemId)
            print(sql)
            cursor.execute(sql)
            db.commit()
