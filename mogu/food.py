...
def main():
    ...
    cursor = db.cursor()
    for k in range(1000):
        url = 'https://list.mogujie.com/search?callback=jQuery21107121902971988092_1540373739282&_version=8193&ratio=3%3A4&cKey=15&page=3&sort=pop&ad=0&fcid=52014&action=food'+ str(k+1)+str('&sort=pop&ad=0&fcid=52014&action=food')
        ...
        for i in range(len(docs)):
            ...
            sql = """insert into food (title,orgPrice,sale,cfav,price,tradeItemId) values('%s','%s','%s','%s','%s','%s')""" % (title, orgPrice, sale, cfav, price, tradeItemId)
            print(sql)
            cursor.execute(sql)
            db.commit()
