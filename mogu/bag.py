...
def main():
    ...
    cursor = db.cursor()
    for k in range(1000):
        url = 'https://list.mogujie.com/search?callback=jQuery21109124849757774802_1540372317784&_version=8193&ratio=3%3A4&cKey=15&page='+ str(k+1)+str('&sort=pop&ad=0&fcid=50675&action=bags')
        ...
        for i in range(len(docs)):
            ...
            sql = """insert into bag (title,orgPrice,sale,cfav,price,tradeItemId) values('%s','%s','%s','%s','%s','%s')""" % (title, orgPrice, sale, cfav, price, tradeItemId)
            print(sql)
            cursor.execute(sql)
            db.commit()
