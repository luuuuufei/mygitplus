...
def main():
    ...
    cursor = db.cursor()
    for k in range(1000):
        url = 'https://list.mogujie.com/search?callback=jQuery21109935862563961009_1540370469372&_version=8193&ratio=3%3A4&cKey=15&page='+ str(k+1)+str('&sort=pop&ad=0&fcid=50004&action=skirt')
        ...
        for i in range(len(docs)):
            if result['result']['wall']['isEnd']:
                ...
                sql = """insert into skirt (title,orgPrice,sale,cfav,price,tradeItemId) values('%s','%s','%s','%s','%s','%s')""" % (title, orgPrice, sale, cfav, price, tradeItemId)
                print(sql)
                cursor.execute(sql)
                db.commit()
    return None
